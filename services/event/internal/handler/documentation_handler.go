...
func (h *documentationHandler) RegisterDocumentation(ctx *gin.Context) {
	...
	documentationUseCaseDTO := mapping.DocumentationRequestToUsecaseDTO(&documentationRequest, adminID)
	//register event
...
func (h *documentationHandler) GetEventByID(ctx *gin.Context) {
	defer recovery.Recover(ctx)
	...
	if id == "" {
		...
		ctx.JSON(http.StatusBadRequest, res.JSON(false, "Failed to get documentation", customErr))
		return
	}

	documentationDTO, err := h.documentationUseCase.GetByID(id, ctx)
	if err != nil {
		if customErr, ok := err.(*customerror.Err); ok {
			ctx.JSON(http.StatusBadRequest, res.JSON(false, "Failed to get documentation", customErr))
			return
	...
	result := response.DetailDocumentation(documentationDTO)
